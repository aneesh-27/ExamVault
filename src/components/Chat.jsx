  ...
  return (
    <div className="space-y-2 text-sm leading-relaxed">
      {lines.map((line, i) => {
        ...
        if (line.startsWith("- ")) return <div key={i} className="flex items-start gap-2"><span className="mt-1.5 w-1 h-1 rounded-full flex-shrink-0" style={{ background: "var(--ink)" }} />{line.slice(2)}</div>;
        if (line.startsWith("FORMULA:")) return <div key={i} className="sa-serif text-sm px-3 py-2 rounded-lg my-1" style={{ background: "#F4F2EC" }}>{line.slice(8)}</div>;
    ...
    <div className="sa-fade-in grid md:grid-cols-[220px_1fr] gap-5 h-[calc(100vh-160px)]">
      <div className="sa-card p-4 h-fit hidden md:block">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-sm font-medium">Computer Networks.pdf</span>
        </div>
        <div className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--muted)" }}>Topic</div>
        <select className="w-full px-2 py-2 rounded-lg text-sm outline-none" style={{ border: "1px solid var(--line)" }}>
      ...
      <div className="sa-card sa-card-lg flex flex-col overflow-hidden">
        <div ref={scrollRef} className="flex-1 overflow-y-auto sa-scroll p-5 space-y-4">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[85%] rounded-2xl px-4 py-3 ${m.role === "user" ? "text-white" : ""}`}
                style={m.role === "user" ? { background: "var(--ink)", borderBottomRightRadius: 4 } : { background: "#F4F2EC", borderBottomLeftRadius: 4 }}>
          ...
          {typing && (
            <div className="flex justify-start">
              <div className="rounded-2xl px-4 py-3 flex gap-1" style={{ background: "#F4F2EC" }}>
        ...
        <div className="px-5 pb-2 flex flex-wrap gap-2">
          {quickTools.map((t) => (
            <button key={t} onClick={() => send(t + ": " + (messages.filter(m => m.role === 'user').slice(-1)[0]?.text || "this topic"))}
              className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: "var(--accent-soft)", color: "var(--accent-ink)" }}>
              {t}
            </button>
          ))}
        </div>
        ...
        <div className="p-4 border-t flex items-center gap-2" style={{ borderColor: "var(--line)" }}>
          <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()}
            ...
            className="flex-1 px-4 py-2.5 rounded-full text-sm outline-none" style={{ border: "1px solid var(--line)" }} />
