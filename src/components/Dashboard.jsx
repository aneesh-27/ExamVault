...
function Dashboard({ go, showToast }) {
  ...
  return (
    <div className="sa-fade-in space-y-6">
      <div>
        <h2 className="sa-serif text-2xl font-semibold">Good morning, Aarav 👋</h2>
        ...
        {stats.map((s) => (
          <div key={s.label} className="sa-card p-4">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-3"
              style={{ background: s.tone === "accent" ? "var(--accent-soft)" : s.tone === "success" ? "var(--success-soft)" : s.tone === "danger" ? "var(--danger-soft)" : s.tone === "info" ? "#E5EEF7" : "#EFEDE6" }}>
              <s.icon size={15} color={s.tone === "accent" ? "var(--accent-ink)" : s.tone === "success" ? "var(--success)" : s.tone === "danger" ? "var(--danger)" : s.tone === "info" ? "var(--info)" : "var(--muted)"} />
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 sa-card sa-card-lg p-5" style={{ background: "var(--ink)" }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <span className="text-xs font-medium" style={{ color: "#9BA4BE" }}>Continue learning</span>
          ...
          <div className="flex items-center justify-between mt-5">
            <span className="text-xs" style={{ color: "#9BA4BE" }}>Next: Laplace Transforms · 40% mastery</span>
          ...
          <div className="grid grid-cols-2 gap-2">
            {quickActions.map((a) => (
              <button key={a.label} onClick={() => go(a.page)}
                className="sa-btn-outline flex flex-col items-start gap-2 p-3 text-left">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => go("documents")} className="text-xs font-medium flex items-center gap-1" style={{ color: "var(--info)" }}>
          ...
          {DOCUMENTS.slice(0, 4).map((d) => (
            <div key={d.id} className="sa-card p-4 flex flex-col">
              <div className="flex items-start gap-2 mb-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "#EFEDE6" }}>
              ...
              <div className="flex gap-2 mt-auto">
                <button onClick={() => go("documents")} className="sa-btn-outline flex-1 text-xs py-1.5 font-medium">Study</button>
